...
#%%
#Approach 2 - assign keys and values to dictionary A only when needed 
#This is used esp. when C is large and/or n is small
def Knapsack2(values, sizes, C, fullArray=False):
    if len(values)!=len(sizes):
        print("Error, number of values does not mach number of sizes.")
        return
    n = len(values)
    A = {}   #use dictionary instead of array to store results (array size can explode for very large C)
    for i in range(n+1):
        A[i] = {}
...
def fill(A, i, c, values, sizes):   #fill A[i][c] based on values, sizes and selected elements in A
    if i==0 or c<=0:
        A[i][c] = 0   #base case
        return
    if c not in A[i-1].keys():   #if A[i-1][c] not yet calculated
        fill(A,i-1,c,values,sizes)   #call fill recursively
    if sizes[i-1]>c:
        A[i][c] = A[i-1][c]
    else:
        if c-sizes[i-1] not in A[i-1]:   #if A[i-1][c-sizes[i-1]] not yet calculated
...
%time Knapsack(V,S,C)
%time Knapsack2(V,S,C)   #approach 2 is much faster in this case
...
V = sample(range(1000,50000),n)
S = sample(range(10000,100000),n)
Knapsack2(V,S,C)   #approach 1 cannot handle this case
